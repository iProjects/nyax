...
namespace nthareneapi
{
	...
	public class postgresqlapisingleton {
		...
		private string db_name = DBContract.DATABASE_NAME;//"ntharenedb";
...
//				updateexistingdbschema();
		...
		public bool createdatabase(){
			try{
				string _default_database_name = utilzsingleton.getInstance(_notificationmessageEventname).getappsettinggivenkey("postgresql_database", "ntharenedb");
		...
		public bool createcropdiseaseindatabase(cropdiseasedto _cropdiseasedto){
			...
			const string query = "INSERT INTO tblcropsdiseases(crop_disease_name, crop_disease_status, created_date) VALUES(@crop_disease_name, @crop_disease_status, @created_date)"; 
...
//				MessageBox.Show(ex.Message,"crop insert",MessageBoxButtons.OK,MessageBoxIcon.Error);
	    ...
	    public responsedto createtables(postgresqlconnectionstringdto _connectionstringdto) {
	        ...
	        //-- serial data type is auto incrementing four-byte integer
	         string SQL_CREATE_CROPS_TABLE = "CREATE TABLE IF NOT EXISTS " + DBContract.cropsentitytable.CROPS_TABLE_NAME + " (" +
	                DBContract.cropsentitytable.CROP_ID + " SERIAL PRIMARY KEY NOT NULL, " +
	          ...
	          if(_innerresponsedto.isresponseresultsuccessful)
				...
				string SQL_CREATE_CROPS_VARIETIES_TABLE = "CREATE TABLE IF NOT EXISTS " + DBContract.cropsvarietiesentitytable.CROPS_VARIETIES_TABLE_NAME + " (" +
		                DBContract.cropsvarietiesentitytable.CROP_VARIETY_ID + " SERIAL PRIMARY KEY NOT NULL, " +
	        ...
	        //Create the crops diseases/pests table
	         string SQL_CREATE_CROPS_DISEASES_TABLE = "CREATE TABLE IF NOT EXISTS " + DBContract.cropsdiseasesentitytable.CROPS_DISEASES_TABLE_NAME + " (" +
	                DBContract.cropsdiseasesentitytable.CROP_DISEASE_ID + " SERIAL PRIMARY KEY NOT NULL, " +
	        ...
	        //Create the manufacturers table
	         string SQL_CREATE_MANUFACTURERS_TABLE = "CREATE TABLE IF NOT EXISTS " + DBContract.manufacturersentitytable.MANUFACTURERS_TABLE_NAME + " (" +
	                DBContract.manufacturersentitytable.MANUFACTURER_ID + " SERIAL PRIMARY KEY NOT NULL, " +
	        ...
	        //Create the pesticides/insecticides table
	         string SQL_CREATE_PESTSINSECTICIDES_TABLE = "CREATE TABLE IF NOT EXISTS " + DBContract.pestsinsecticidesentitytable.PESTSINSECTICIDES_TABLE_NAME + " (" +
	                DBContract.pestsinsecticidesentitytable.PESTINSECTICIDE_ID + " SERIAL PRIMARY KEY NOT NULL, " +
	        ...
	        //Create the settings table
	         string SQL_CREATE_SETTINGS_TABLE = "CREATE TABLE IF NOT EXISTS " + DBContract.settingsentitytable.SETTINGS_TABLE_NAME + " (" +
	                DBContract.settingsentitytable.SETTING_ID + " SERIAL PRIMARY KEY NOT NULL, " +
	        ...
	        //Create the categories table
	         string SQL_CREATE_CATEGORIES_TABLE = "CREATE TABLE IF NOT EXISTS " + DBContract.categoriesentitytable.CATEGORIES_TABLE_NAME + " (" +
	                DBContract.categoriesentitytable.CATEGORY_ID + " SERIAL PRIMARY KEY NOT NULL, " +
		          ...
		          if(_innerresponsedto.isresponseresultsuccessful)
					  ...
					  catalogsenum _catalogsenum = new catalogsenum();
					  ...
					  if(_responsedto.responseerrormessage != null){
					   
					 	 if(_responsedto.responseerrormessage.Length > 0){
					  	
			         	 	_responsedto.responsesuccessmessage += _responsedto.responseerrormessage;
		...
		public responsedto checkconnectionasadmin(postgresqlconnectionstringdto _connectionstringdto){
			...
			try{
			    ...
			    if(count != -1){
			    	_responsedto.isresponseresultsuccessful = true;
					_responsedto.responsesuccessmessage = "connection to postgresql successfull. crops count [ " + count + " ].";
			    	return _responsedto;
			    }else{
			    	_responsedto.isresponseresultsuccessful = false;
